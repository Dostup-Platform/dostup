    ...
    <Dialog open={!!pausingProduct} onOpenChange={(open) => { if (!open) setPausingProduct(null); }}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3 text-sm text-muted-foreground">
          <p>
            {language === "ru"
              ? "Ссылка на продукт, которую вы отправляете ученикам, станет недействительной. По этой ссылке можно будет перейти, но вместо кнопки покупки посетители увидят сообщение ниже."
